...
export class RampUp extends Metric {
    ...
    async calculateScore(): Promise<void> {
        ...
        try {
            const response = await axios.get(apiEndpoint, {headers: {'Authorization': `token ${secret('GITHUB_TOKEN')}`}});
            ...
            if(docs_folder) {
                const documents = await axios.get(`${apiEndpoint}/${docs_folder}`, {headers: {'Authorization': `token ${secret('GITHUB_TOKEN')}`}});
                documents.data.forEach((doc: any) => {
                    total_documentation_size += doc.size;
                });
            }
            if(examples_folder) {
                const examples = await axios.get(`${apiEndpoint}/${examples_folder}`, {headers: {'Authorization': `token ${secret('GITHUB_TOKEN')}`}});
